...
function addon:ADDON_LOADED(event, addon)
	if addon == "PhotoRobot" then
		DEFAULT_CHAT_FRAME:AddMessage("|c00FF9900[PT]: |rPhotoRobot v"..self.version.." Loaded")
...
function addon.CheckAura(unit)
	...
	for idx, spellID in ipairs(addon.spellDB) do
		...
		if (spell ~= nil) then
			...
			for i = 1, #auCheck do						
				--[[if (name ~= nil) then 
					print("checking aura #" .. i .. " named " .. name) 
					uat = {UnitAura(unit, i)}
					print(#uat)
					for i = 1, #uat do
						local s = "nil"
					...
					if eTime ~= nil then
						print("Expiration: " .. eTime .. " seconds");
					end
				end]]
	...
	if (#auras > 0) then	
		...
		for i, au in ipairs(auras) do
			...
			if not flag then
				...
				--if not au.unit then print("Unit is NIL!!! Galactics is in danger!!!") end
